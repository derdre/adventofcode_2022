...
local function do_single_move(stack, from, to)
    local letter_transit = table.remove(stack[from])
    table.insert(stack[to], letter_transit)
    return stack
end

local function process_instruction(stack, move, from, to)
    for _ = 1, move do
        stack = do_single_move(stack, from, to)
    end
    return stack
end
...
local function main()
    ...
    for i = 1, #instructions do
        ...
        stack_process = process_instruction(stack_process, move, from, to)
        -- tprint(stack)
    end
